# Variables for require-tag-environment policy
...
variable "policy_assignment_name" {
  ...
  type        = string
  default     = "require-tag-env-assignment"
  validation {
...
variable "policy_assignment_display_name" {
  ...
  type        = string
  default     = "Require Environment Tag Assignment"
}

variable "policy_assignment_description" {
  description = "Description for the policy assignment"
  type        = string
  default     = "This assignment enforces the policy to require an Environment tag on all resources."
}
...
variable "allowed_values" {
  ...
  validation {
    condition     = length(var.allowed_values) > 0
    error_message = "At least one allowed value must be specified."
  }
}
