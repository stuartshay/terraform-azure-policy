# Variables for require-tag-costcenter policy
...
variable "policy_assignment_name" {
  ...
  type        = string
  default     = "require-tag-costcenter-assignment"
  validation {
...
variable "policy_assignment_display_name" {
  ...
  type        = string
  default     = "Require CostCenter Tag Assignment"
}

variable "policy_assignment_description" {
  description = "Description for the policy assignment"
  type        = string
  default     = "This assignment enforces the policy to require a CostCenter tag on all resources."
}
...
variable "tag_pattern" {
  description = "Regular expression pattern that the CostCenter tag value must match"
  type        = string
  default     = "^CC-[0-9]{4,6}$"
  validation {
    condition     = can(regex(var.tag_pattern, "CC-1234"))
    error_message = "The tag_pattern must be a valid regular expression."
  }
}
