# Variables for the Function App AAD Only Policy Module
...
variable "policy_assignment_name" {
  ...
  type        = string
  default     = "deny-function-app-aad-only-assignment"
}

variable "policy_assignment_display_name" {
  description = "Display name for the policy assignment"
  type        = string
  default     = "Deny Function App Without AAD-Only Authentication Assignment"
}

variable "policy_assignment_description" {
  description = "Description for the policy assignment"
  type        = string
  default     = "This assignment enforces the policy to deny Function Apps that do not have Azure AD-only authentication enabled with FTPS and basic publishing credentials disabled."
}
