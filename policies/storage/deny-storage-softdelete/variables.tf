# Variables for deny-storage-softdelete policy
...
variable "policy_assignment_name" {
  ...
  type        = string
  default     = "deny-storage-softdelete-assignment"
  validation {
...
variable "policy_assignment_display_name" {
  ...
  type        = string
  default     = "Deny Storage Account Soft Delete Disabled Assignment"
}

variable "policy_assignment_description" {
  description = "Description for the policy assignment"
  type        = string
  default     = "This assignment enforces the policy to deny storage accounts with soft delete disabled or insufficient retention periods."
}
...
variable "minimum_retention_days" {
  ...
  default     = 7
  validation {
    condition     = var.minimum_retention_days >= 1 && var.minimum_retention_days <= 365
    error_message = "Minimum retention days must be between 1 and 365."
  }
}
